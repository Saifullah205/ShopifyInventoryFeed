...
namespace ShopifyInventorySync.BusinessLogic.Walmart
{
    internal class WalmartThePerfumeSpot
    {
        ...
        public List<ThePerfumeSpotProduct> FilterProductsToRemove(ThePerfumeSpotProductsList productsList)
        {
            ...
            List<RestrictedBrand> restrictedBrands = new();
            List<RestrictedSku> restrictedSku = new();
            ...
            try
            {
                ...
                productsToRemove = (from s in productsList.products
                                    where restrictedSku.Any(x => x.Sku == s.UPC) || restrictedBrands.Any(x => x.BrandName!.Trim().ToUpper() == s.Brand.Trim().ToUpper()) || restrictedTerms.Any(x => s.Name.Split(',')[0].Trim().ToUpper().Contains(x.Term!.Trim().ToUpper()))
                                    select s).ToList();
